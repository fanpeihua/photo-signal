...
- 方向与差距：按实际评论生成本期决策简报，导出候选与原文，一键将假设和证据带入实践；公开产品基线、竞品声音、主题实践与验收方式；可编辑自己的能力基线。
...
- 决策简报只使用近 30 天有效发布时间与评分，按 ID 去重；同品牌至少 2 条低分评论命中付费、稳定性、画质或导出词组时才形成候选。词语命中不证明因果或真实缺陷，必须核对原文；评论数不等于独立用户数，分组可重叠。两段连续 7 天分别列出负向数和评分总数，任一段不足 10 条时不比较比例；即使样本够也不推断全网趋势。Kapi 候选优先展示，竞品抱怨只能提示对照实测，不能直接当作本品差距。
...
`python3 check.py` 验证去重、首见保留、未知/未来日期、评分口径、私有数据隔离、导出/恢复历史和内网地址拒绝。`node check-metrics.js` 用全黑、全白和棋盘图验证指标；`node check-insights.js` 验证决策候选的去重、品牌隔离、日期边界和样本分母；`node --check web/app.js` 验证语法。真实接口和网页交互还需运行核验，测试通过不代表所有平台已接通。
