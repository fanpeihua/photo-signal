...
`browser-extension/` 是可审阅的 Manifest V3 扩展，零依赖、默认停止。它使用当前 Chrome 中的小红书登录状态，每两小时打开自己的后台搜索页，读取「摄影 / 咔皮相机 / Dazz / ProCCD」各最多 30 张搜索卡片；综合排序包含历史内容，不等于最新热榜。登录、验证码或页面变化会导致本轮失败，扩展弹窗显示结果并保留旧样本。

采样只保存标题、去掉所有查询参数的笔记链接、日期/点赞显示值和采样时间。不会读取 Cookie、账号 ID、私信、通知、评论正文、密码或完整笔记。没有上传端点；数据存于扩展的本机存储，并发送给当前浏览器打开的取景页面。页面把它与公共快照合并显示，但不写入 GitHub 或云端。公开网站的其他访问者看不到这些本机样本。

安装需用户确认 Chrome 扩展权限：仅小红书域名、取景 GitHub Pages 域名和本机 `127.0.0.1:8840`，以及定时器和本机存储；不请求 cookies、history、所有网站或调试器权限。内容脚本仅在小红书搜索页和取景路径工作。Chrome 的 host 权限按域名授权，页面路径由内容脚本配置进一步限定。

1. 在 `chrome://extensions` 开启开发者模式，选择「加载已解压的扩展程序」，选本仓库 `browser-extension` 文件夹。
2. 保持小红书已登录，打开扩展弹窗并点击「开启两小时采样」。看到本轮实际条数才代表采样成功。
3. 打开或刷新取景网站，进入「来源与覆盖」核对「浏览器采样」和时间。也可用扩展导出的 JSON 在网站导入。
4. 「停止」会清除定时器；电脑休眠、Chrome 关闭或登录过期时不能承诺准点执行。[Chrome 定时器规则](https://developer.chrome.com/docs/extensions/reference/api/alarms)。

当前已验证：登录后的真实卡片读取、导入去重与隐私字段过滤。扩展安装、首次采样和跨两小时时间段的自动更新，需要安装后另行验证；不能把代码准备完成当成已经定时运行。
